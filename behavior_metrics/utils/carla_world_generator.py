...
def read_data_from_yaml(yaml_file_path):
    """
    Reads the robot data, the type of vehicle and sensors, from a YAML file.
    """
    try:
        with open(yaml_file_path, 'r') as file:
            ...
            if 'Behaviors' in data:
                ...
                print("Warning: 'Behaviors' key not found in the YAML file.")
                return None
...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        self.sync = not args['Robot']['AsyncMode']
        self.actor_role_name = args['Robot']['Name']
        try:
    ...
    def restart(self):
        ...
        if self._blueprint is None:
            blueprint_list = blueprint_library.filter('vehicle.*')   
            if not blueprint_list:
                raise ValueError("Couldn't find any blueprints with the 'vehicle' filter in the selected world")
            blueprint = random.choice(blueprint_list)
        else:
            blueprint = blueprint_library.find(self._blueprint)
        
        # Set the main vehicle atributes
        ...
        if blueprint.has_attribute('speed'):
            blueprint.set_attribute('speed',float(self.max_speed))
    ...
    def spawn_cameras(self,bp_library):

        for camera in self._cams_blueprints:
            try:
                camera_bp = bp_library.find('sensor.camera.'+ camera['Type'])
                camera_bp.set_attribute('role_name', camera['Topic'])
                camera_position = [float(coord) for coord in camera['Position'].split()]
                camera_transform = self.list_to_transform(camera_position)
                camera_object = self.world.spawn_actor(camera_bp, camera_transform, attach_to=self.player)
                self.camera_list.append(camera_object)
                print('Created camera: '+ camera['Name'])
            except Exception:
                print('Unable to create camera: ' + camera['Name'])
            
    def list_to_transform(self, list):

        transform = carla.Transform(carla.Location(x=list[0], 
                                                   y=list[1],
                                                   z=list[2]),
                                    carla.Rotation(pitch=list[3], 
                                                   yaw=list[4],
                                                   roll=list[5]))
        return transform              
...
def game_loop(data):
    ...
    try:
        ...
        world = World(sim_world, hud, data)
...
def main(file_path):
    try:
        sim_data = read_data_from_yaml(file_path)
        game_loop(sim_data)
