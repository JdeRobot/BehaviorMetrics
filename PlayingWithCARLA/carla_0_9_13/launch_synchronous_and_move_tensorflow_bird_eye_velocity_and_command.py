...
def main():
    ...
    try:
        ...
        model = "20221109-111804_pilotnet_CARLA_17_10_dataset_bird_eye_300_epochs_no_flip_prev_velocity_and_command_town_01_cp.h5"
        ...
        with CarlaSyncMode(world, camera_rgb, camera_semseg, fps=15) as sync_mode:
            while True:
                ...
                # Straight command!
                command_dim = np.full((150, 50), 1)
                new_img_vel = np.dstack((img, velocity_dim))
                new_img_vel_cmd = np.dstack((new_img_vel, command_dim))
                img = new_img_vel_cmd
