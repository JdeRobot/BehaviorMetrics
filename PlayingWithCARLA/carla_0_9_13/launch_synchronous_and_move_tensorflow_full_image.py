...
def draw_image(surface, image, blend=False, location=(0,0)):
    try:
        ...
        array = image
        array = cv2.resize(array, (800, 266))    
    array = array[:, :, ::-1]
...
def main():
    ...
    display = pygame.display.set_mode((800*2, 600),pygame.HWSURFACE | pygame.DOUBLEBUF)
    font = get_font()
    ...
    try:
        ...
        start_pose = carla.Transform(carla.Location(x=-2.0, y=285, z=1.37), carla.Rotation(pitch=0, yaw=90, roll=0))
        ...
        PRETRAINED_MODELS = "../../../"
        model = "20221024-110444_pilotnet_CARLA_24_10_dataset_full_image_300_epochs_no_flip_3_output_no_sync_cp.h5"
        model = "20221024-114401_pilotnet_CARLA_24_10_dataset_full_image_300_epochs_no_flip_3_output_more_data_cp.h5"
        model = "20221024-123807_pilotnet_CARLA_24_10_dataset_full_image_300_epochs_no_flip_3_output_more_data_no_sync_cp.h5"
        model = "20221024-132531_pilotnet_CARLA_24_10_dataset_full_image_300_epochs_no_flip_3_output_more_data_no_sync_extreme_cp.h5" #BEST!
        #model = "20221026-161920_pilotnet_CARLA_24_10_dataset_full_image_300_epochs_no_flip_3_output_more_data_no_sync_extreme_cp.h5"
        model = "20221026-163028_pilotnet_CARLA_24_10_dataset_full_image_300_epochs_no_flip_3_output_more_data_no_sync_extreme_cp.h5" # Weather!

        net = tf.keras.models.load_model(PRETRAINED_MODELS + model)

        # Create a synchronous mode context.
        with CarlaSyncMode(world, camera_rgb, camera_semseg, fps=5) as sync_mode:
            while True:
                if should_quit():
                    return
                start = time.time()
                ...
                if vehicle_speed > 20:
                    ...
                    if vehicle_speed > 20 and break_command > float(0.01):
                        ...
                        print('entra')
                        vehicle.apply_control(carla.VehicleControl(throttle=float(0.5), steer=steer, brake=float(0.0)))
                    else:
                ...
                draw_image(display, output, blend=False, location=(800,266))
                display.blit(
